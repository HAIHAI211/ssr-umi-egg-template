 ...
 * @LastEditTime: 2021-01-06 11:09:50
 * @Description: http请求接口页
 */

import { HTTP, serviceDeconstruct, getIdUrl } from "./config";
import qs from "qs";
...
export async function fetchJson({ dir }: IFetchJsonProps) {
  try {
    ...
    const result = await axios.get(
      `/${IS_PROD ? "prod" : "test"}/origin${dir}/data.json?t=${timestamp}`,
      {
...
export async function sendCode(props: ISendCodeProps) {
  const { code, data } = await serviceDeconstruct(
    ...
    // }),
    {
      successCodes: [0, 100, 200, 400, 600, 700, 800, 900],
    }
  );
...
export function fetchLogin(
  loginProps: IFetchLoginProps
): Promise<IReturn<IFetchLoginReturn>> {
  return serviceDeconstruct(
    HTTP({
      method: "POST",
      baseURL: getIdUrl(),
    ...
    // 			msg: '验证码错误',
    // 			data: null
    // 			// {"code":10301,"msg":"验证码错误","data":null}
    // 		}
    // 	})
    // })
  );
